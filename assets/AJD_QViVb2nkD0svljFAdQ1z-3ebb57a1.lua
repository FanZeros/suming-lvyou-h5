...
local darkBgDebugLogged_ = false
local imgDarkBg_ = -1                 -- LOCAL_MODE 暗黑夜空背景（替代背景视频）
...
function StartScreen.init(nvgCtx, scene)
    ...
    -- 🔴 LOCAL_MODE（单机暗黑画风）：加载暗黑夜空背景，替代明亮背景视频
    if _G.LOCAL_MODE then
        imgDarkBg_ = nvgCreateImage(vg_, "image/dark_title_bg.png", 0)
        if imgDarkBg_ >= 0 then
            print("[StartScreen][LOCAL] dark title background loaded")
        end
    end
    ...
    -- 背景视频（循环、静音）——LOCAL_MODE 下不加载（用暗黑静态背景替代）
...
function StartScreen.draw(vg)
    ...
    -- 1. 背景：LOCAL_MODE 优先用暗黑夜空图，否则背景视频（全屏）/ 始终全不透明，不参与淡出
    if _G.LOCAL_MODE and imgDarkBg_ >= 0 then
        if not darkBgDebugLogged_ then
            darkBgDebugLogged_ = true
            print("[StartScreen][LOCAL] drawing dark bg, handle=" .. tostring(imgDarkBg_))
        end
        drawImg(vg, imgDarkBg_, DESIGN_W * 0.5, DESIGN_H * 0.5, DESIGN_W, DESIGN_H, 1.0)
    elseif not videoHandle_ and videoPlayer_ and videoPlayer_:IsReady() then
        local texture = videoPlayer_:GetTexture()
        if texture and nvgCreateVideo then
            videoHandle_ = nvgCreateVideo(vg, texture)
        end
    end
    if not (_G.LOCAL_MODE and imgDarkBg_ >= 0) then
        -- 非暗黑主题（或暗图加载失败）才画视频/fallback，否则会盖掉暗黑背景
    ...
    -- 7. 区服状态(背景 cx=540 cy=1854 600×70)——LOCAL_MODE 单机不显示
    if not _G.LOCAL_MODE then
    do
        -- 7a. 半透明圆角背景
...
function StartScreen.handleClick(dx, dy)
    ...
    -- 点击区服状态区域(cx=540 cy=1854 600×70) → 打开选服面板（LOCAL_MODE 单机不响应）
    local bgCx, bgCy = 540, 1854
    local bgW, bgH   = 600, 70
    local hitServer = dx >= bgCx - bgW * 0.5 and dx <= bgCx + bgW * 0.5
        and dy >= bgCy - bgH * 0.5 and dy <= bgCy + bgH * 0.5
    if hitServer and not _G.LOCAL_MODE then
...
function StartScreen.reopen(scene)
    ...
    -- 重新创建视频播放器（LOCAL_MODE 单机不加载背景视频——文件已从包中剔除）
    if not _G.LOCAL_MODE then
        videoPlayer_ = VideoPlayer:new()
        if videoPlayer_ then
            local ok = videoPlayer_:Load("video/UI_DLJMBJ_Compat.mp4", 1080, 2400)
