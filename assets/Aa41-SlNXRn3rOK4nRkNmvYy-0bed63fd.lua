...
function ScenarioDialogue.show(config)
    -- 🔴 LOCAL_MODE（单机）：剧情弹窗整体停用——开场/里程碑情景不再打断游玩。
    -- 有 onFinish 链的情景直接续链（如轮回前置情景 → CG 流程照常往下走）。
    if _G.LOCAL_MODE then
        print("[ScenarioDialogue][LOCAL] popup skipped")
        if config and type(config.onFinish) == "function" then
            config.onFinish()
        end
        return
    end
