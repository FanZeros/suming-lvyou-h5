...
local ThemeOverlay       = require("ui.ThemeOverlay")
...
--- 统一选服入口：面板信息 + 状态迁移 + 发送 SELECT_SERVER
--- 被 StartScreen 选服回调（用户手选）与 LOCAL_MODE 自动选服共用
local function selectServerNow(serverId)
    print("[Client] select server id=" .. tostring(serverId))
    -- 设置当前区服名称到玩家信息面板（LOCAL_MODE 单机不显示区服信息）
    local serverCfg = ServerListConfig.find(serverId)
    if serverCfg then
        if not _G.LOCAL_MODE then
...
local function handleServerList(eventType, eventData)
    ...
    -- 🔴 LOCAL_MODE（单机回环）：没有"选服务器"概念——收到列表立即自动选服，
    -- 复用与手选完全相同的状态机（selectServerNow），面板永不强制弹出。
    -- 默认沿用上次存档区（lastServerId），全新玩家落到本地固定存档区 11。
    if _G.LOCAL_MODE then
        -- 固定本地存档区 11（不 require LocalHost，避免循环依赖；回环入口恒为 11）
        local sid = (data.lastServerId and data.lastServerId > 0) and data.lastServerId or 11
        selectServerNow(sid)
        print("[Client][LOCAL] auto-selected server " .. tostring(sid) .. " (panel skipped)")
        return
    end
...
function Client.Start()
    ...
    -- 3.9 暗黑画风主题层（LOCAL_MODE 生效；每帧渲染末尾绘制全局氛围）
    ThemeOverlay.init()
...
function HandleNanoVGRender_Client(eventType, eventData)
    ...
    nvgBeginPath(vg)
    nvgRect(vg, 0, 0, screenDesignW, screenDesignH)
    nvgFillColor(vg, nvgRGBA(ThemeConfig.CLEAR_R, ThemeConfig.CLEAR_G, ThemeConfig.CLEAR_B, 255))
    nvgFill(vg)
  ...
  local _ok, _err = pcall(function()
    ...
    if currentState == STATE_IN_GAME then
        ...
        if loadingScreenWasOpen_ and not LoadingScreen.isOpen() then
            ...
            if _G.LOCAL_MODE then
                -- 🔴 单机模式：跳过开场动画与情景对话，新档直接进入选角
                GameBGM.start()
                GameSFX.start()
                if hasReincarnated then
                    -- 轮回标记直接清除（不补播入场动画）
                    Client.sendAction(Protocol.ACTION_TYPES.CLEAR_REINCARNATION, {})
                end
                if rosterEmpty then
                    print("[Client][LOCAL] skip intro story, straight to character select")
                    CharacterSelect.show({
                        background = "image/关卡地图/MAP_1.png",
                        onFinish = function(heroId)
                            print("[Client][LOCAL] character selected: heroId=" .. heroId)
            ...
            elseif rosterEmpty and not IntroCutscene.isFinished() then
